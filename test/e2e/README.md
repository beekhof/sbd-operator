...
E2E tests use the **generic Kubernetes webhook configuration** (`config/default`) rather than the OpenShift service-ca approach. This allows the tests to work with manually generated self-signed certificates that are created by `scripts/generate-webhook-certs.sh`.

**Key differences:**
- **OpenShift deployments**: Use service-ca for automatic certificate management
- **E2E tests**: Use self-signed certificates generated by the test script
...
**3. Webhook Certificate Not Found**
```
...
**Solution**: E2E tests use `config/default` (generic) instead of `config/openshift-default` (service-ca) to work with manually generated certificates.
